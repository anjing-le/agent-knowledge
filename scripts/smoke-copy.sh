...
TMP_ROOT="${TMPDIR:-/tmp}"
WORK_DIR="$(mktemp -d "$TMP_ROOT/agent-knowledge-copy-smoke.XXXXXX")"
...
walk('.', (file) => {
  ...
  next = replaceAll(next, 'agent-knowledge', projectName)
  next = replaceAll(next, 'Agent Knowledge', projectName)
  next = replaceAll(next, 'agent_knowledge', dbName)
...
if rg -n 'agent-dev-scaffolding|apifoxmock|6400575|6097373|Daymychen/art-design-pro' \
...
if rg -n 'Agent Dev Scaffolding|UnoCSS|agent-dev-scaffolding|apifoxmock|6400575|6097373|Daymychen/art-design-pro' \
