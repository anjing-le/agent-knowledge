...
function extractJavaConstants(source, className) {
  ...
  for (const match of source.matchAll(/public static final String\s+(\w+)\s*=\s*([^;]+);/g)) {
    const firstNestedClassIndex = source.search(/public static (?:final )?class/)
    if (match.index < firstNestedClassIndex) {
      globalConstants[match[1]] = evaluateJavaExpression(match[2], globalConstants)
    }
  }

  const markerMatch = source.match(new RegExp(`public static (?:final )?class ${className}`))
  if (!markerMatch) {
    fail(`missing ApiConstants.${className} module block`)
  }

  const block = findBlock(source, markerMatch[0])
