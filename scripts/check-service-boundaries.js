...
function extractJavaConstants(source, className) {
  ...
  const firstNestedClass = source.search(/public static (?:final )?class/)
  for (const match of source.matchAll(/public static final String\s+(\w+)\s*=\s*([^;]+);/g)) {
    if (firstNestedClass < 0 || match.index < firstNestedClass) {
      globalConstants[match[1]] = evaluateJavaExpression(match[2], globalConstants)
    }
  }

  const marker = source.includes(`public static final class ${className}`)
    ? `public static final class ${className}`
    : `public static class ${className}`
  const block = findBlock(source, marker)
