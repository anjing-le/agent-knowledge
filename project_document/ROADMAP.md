...
- 检索已具备 score 解释、pgvector vector provider 骨架、本地 hybrid retrieval、本地 keyword provider、Elasticsearch keyword provider 骨架、本地 lexical rerank、远程 rerank provider 边界、`retrieval-adapter-contract.json`、`probe-retrieval-adapters.sh` 切换探针、demo recall@K 评测闭环和前端质量评测面板；后续继续补 Milvus 和 BM25 adapter。
