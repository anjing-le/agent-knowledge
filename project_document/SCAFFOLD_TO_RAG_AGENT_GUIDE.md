...
- 向量检索：`VectorStoreService` 边界、memory 实现、未来 Milvus/pgvector adapter；`RetrievalResultEnrichmentService` 负责把向量命中补全成可引用的 SearchResult；`RetrievalHybridSearchService` 负责本地关键词召回和 RRF 合并；`RetrievalRerankService` 负责本地 lexical rerank 和后续远程 rerank provider 接入点。
