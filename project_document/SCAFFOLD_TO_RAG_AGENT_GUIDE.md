...
6. 在领域服务层拆出阶段服务，例如 `DocumentProcessingService` 负责编排，`DocumentParsingService` 负责解析调用，`DocumentChunkingService` 负责切片生成，`DocumentChunkPersistenceService` 负责切片落库和统计，`DocumentEmbeddingService` 负责向量化和向量写入。
