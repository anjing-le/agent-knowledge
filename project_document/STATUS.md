...
2026-06-17：GitHub Actions runner 补齐 `ripgrep` 依赖，确保依赖 `rg` 的脚手架契约检查能在 CI 和本地使用同一入口。

2026-06-17：前端 auto-import/component 类型声明纳入仓库，保证 clean clone 的 CI 在 `vue-tsc` 阶段也能识别 Vue、Pinia、VueUse 和 Element Plus 自动导入。
...
2026-06-17：准备 `v1.1-teaching-handoff` 交付标签，课前总检查脚本升级为同时确认 `v1-teaching-baseline` 与 `v1.1-teaching-handoff`，README 与 V1 release notes 将当前交付口径收束到 98%。
