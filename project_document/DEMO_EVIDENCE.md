...
12. doc-parser live boundary 通过：`DOC_PARSER_URL=http://localhost:9001 BACKEND_URL=http://localhost:10001 ./scripts/probe-doc-parser-boundary.sh --live`。
13. doc-parser async submit/status live smoke 通过：`./scripts/smoke-doc-parser-async.sh`。
14. 后端健康检查通过：`curl http://localhost:10001/api/test/health`。
15. 前端能打开 RAG Pipeline、知识库列表、知识库详情、切片页、检索调试和智能对话页。
16. RAG Pipeline 页面能说明脚手架地基、RAG 阶段服务、Java/Python 边界，并展示 `Seed -> Evaluate -> Retrieval -> Chat -> Evidence` Demo Ready checklist。
17. Demo Ready 进入检索调试页后，页面自动带入 query/kbIds 并展示 chunk 命中结果。
18. Demo Ready 进入知识问答页后，页面自动带入 query/kbIds、创建会话并展示引用回答。
19. Retrieval Evaluation 面板能展示 recall@K、通过用例数、suite、kbId、每个 query 的 rank/top chunk/score explanation。
20. 聊天答案引用卡能展示 rank、retrievalSource、hybrid/rerank 分数、scoreExplanation、chunk metadata 和查看切片入口。
21. 聊天答案能展示上下文组装 trace，包含 assemblyStrategy、prompt sections、history window、prompt/context 字符数和 included chunks。
22. 上传一份小文档，状态进入完成，切片可查看。
23. 在聊天页选择知识库提问，回答展示引用来源。
...
docs/evidence/YYYY-MM-DD/
  ...
  outputs/
    ...
    probe-doc-parser-boundary-live.txt
...
- Doc-parser live boundary: `DOC_PARSER_URL=http://localhost:9001 BACKEND_URL=http://localhost:10001 ./scripts/probe-doc-parser-boundary.sh --live`
