...
- 工程底座遵守 [../contracts/scaffold-stack-contract.json](../contracts/scaffold-stack-contract.json)：前端 Vue/Vite，后端 Spring Boot/Java，文档解析服务 Python FastAPI。
...
- 分页接口返回 `PageResult<T>`。
- 前端业务 API 优先使用 `openApiRequest(operationId)` 和生成类型；浏览器 `File + FormData` 等 OpenAPI 暂不易表达的接口才使用 `ApiPaths` fallback。
- 前端页面不直接手写 `/api/**`。
