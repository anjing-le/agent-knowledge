...
def load_model(model_dir, nm):
    ...
    if cuda_is_available():
        ...
        # logger.info(f"TextRecognizer {nm} uses GPU")
        logger.info(f"load_model {model_file_path} uses GPU")
    else:
        sess = ort.InferenceSession(
            model_file_path,
            options=options,
    ...
    #     logger.info(f"TextRecognizer {nm} uses CPU")
...
class OCR(object):
    def __init__(self, model_dir=None):
        if not model_dir:
            try:
                ...
                logger.exception("OCR load model failed, please check your model_dir={}".format(e))
        self.drop_score = 0.5
    ...
    def get_rotate_crop_image_v3(self, img, points):
        max_size = 32767  # OpenCV 的最大尺寸限制

        # 检查输入图像尺寸，如果过大则缩小
        img_height, img_width = img.shape[0:2]
        if img_height > max_size or img_width > max_size:
            scale = min(max_size / img_height, max_size / img_width)
            img = cv2.resize(img, (int(img_width * scale), int(img_height * scale)), interpolation=cv2.INTER_AREA)
            img_height, img_width = img.shape[0:2]  # 更新图像尺寸
        ...
        points[:, 0] = points[:, 0] / img_width * max_size
        points[:, 1] = points[:, 1] / img_height * max_size
        ...
        # 如果裁剪后的图像尺寸过大，按比例缩小
        if img_crop_width > max_size or img_crop_height > max_size:
            scale = min(max_size / img_crop_width, max_size / img_crop_height)
            img_crop_width = int(img_crop_width * scale)
            img_crop_height = int(img_crop_height * scale)
        ...
        try:
            ...
            print(f"OpenCV error: {e}. Resizing image and retrying.")
            scale = min(max_size / img_crop_width, max_size / img_crop_height)
            img_crop_width = int(img_crop_width * scale)
            img_crop_height = int(img_crop_height * scale)
            dst_img = cv2.warpPerspective(
    ...
    def get_rotate_crop_image_v2(self, img, points):
        max_size = 800  # OpenCV 的最大尺寸限制
    ...
    def recognize(self, ori_im, box):
        try:
            ...
            try:
                # 尝试使用 v2 版本
                logger.warning(f"get_rotate_crop_image v1 abnormal: {ex}.")
                img_crop = self.get_rotate_crop_image_v2(ori_im, box)
            except Exception as ex:
                logger.error(f"get_rotate_crop_image v2 also failed: {ex}. Returning empty result.")
                return ""  # 返回空字符串作为默认
