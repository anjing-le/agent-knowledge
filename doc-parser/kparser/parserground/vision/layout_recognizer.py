...
from kparser.parserground.vision import Recognizer
from kparser.parserground.vision.operators import nms
...
class LayoutRecognizer(Recognizer):
    ...
    def __init__(self, domain):
        try:
            logger.info("layout recognizer model initializing")
            model_dir = os.path.join(
                    get_project_base_directory(),
                    "rag/res/models")
            super().__init__(self.labels, domain, model_dir)
            logger.info("layout recognizer model load successfully")
        except Exception:
            print("LayoutRecognizer load model failed, please check your model_dir")

        self.garbage_layouts = ["footer", "header", "reference"]

    def __call__(self, image_list, ocr_res, scale_factor=3, thr=0.1, batch_size=16, drop=True):
        def __is_garbage(b):
        ...
        for pn, lts in enumerate(layouts):
            ...
            def findLayout(ty):
                ...
                for lt in lts_:
                    if lt["type"] == "figure":
                        matched = False
                        # 检查bxs中的每个元素是否包含figure的坐标
                        for bx in bxs:
                            if "text" in bx:  # 确保bx是一个包含text的元素
                                if (bx["x0"] <= lt["x0"] and bx["x1"] >= lt["x1"] and
                                        bx["top"] <= lt["top"] and bx["bottom"] >= lt["bottom"]):
                                    matched = True
                                    break
                        ...
                        if not matched:
                            new_bx = {
                                ...
                                'x1': lt['x1'],
                                'top': lt['top'],
                                'text': '',  # 默认文本“示例图片”用于figure
                                'bottom': lt['bottom'],
                                'page_number': lt['page_number']+1,
                                'layout_type': 'figure',
                                'layoutno': f'figure-{lts_.index(lt)}'
                            }
                            bxs.append(new_bx)
